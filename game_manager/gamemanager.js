...
exports.addroom = function (r_roomID, r_title, r_creator, r_seatlimit, r_status, r_game_mode, r_wifi_mode, r_stake_money, r_win_money, socket) {
    let inputplayerlist = [];
    let playerphotos = [];
    let earnScore = [];
    let diceHistory = [];
...
exports.GetUserListInRoom = function (roomid) {
    ...
    for (let i = 0; i < roomlist[roomindex].playerlist.length; i++) {
        mydata = mydata + '{' +
            '"username":"' + roomlist[roomindex].playerlist[i] + '",' +
...
exports.GetRankList = function (socket, info) {
    ...
    collection.find().toArray(function (err, docs) {
        docs.forEach(doc => {
            let points = 0;
            collection_GameHist.find({
                username: doc.username
            }).toArray(function (err, items) {
                if (err)
                    console.log("error _ GameHistory Collection");
                else {
                    for (let index = 0; index < items.length; index++) {
                        if (info.rank_type == 'month') {
                            ...
                            let day = new Date(items[index].date.setDate(items[index].date.getDate())).toISOString().slice(0, 10)
                            //console.log(day);
...
exports.GetTurnUser = function (socket, data) {
    ...
    for (let index = 0; index < roomlist.length; index++) {
        if (roomlist[index].roomid == data.roomid) {
            ...
            if(ischeck.length == 0)
                roomlist[index].turncount.push(username);
...
exports.OnDisconnect = function (socket) {
    ...
    if (ischeck.length == 0) { 
        ...
        if (socket.room == undefined || nickname == undefined)
            return;
                    ...
                    if (isExist == true) {
                        setTimeout(() => {
                            ...
                            if (roomlist[index].playerlist.length == 0) {
                                ...
                                io.sockets.in('r' + roomlist[index].roomid).emit('GAME_END', {});
