        ...
        else
        {
            ...
            collection.insertOne(query,function(err) {
                ...
                else
                {
                    ...
                    gamemanager.addroom(id, userInfo.room_title, userInfo.username, parseInt(userInfo.seat_limit), userInfo.status, 
...
{
    gamemanager.playerenterroom(parseInt(data.roomID),data.username, data.photo, socket);
}
exports.ReJoinRoom = function(socket, data)
{
    gamemanager.reconnectRoom(parseInt(data.roomid),data.username, data.old_socketID, socket);
}
                    ...
                    if(users[0].username == checker)
                    {
                        ...
                        if(ref_users.length > 0)
                        {
                            for (let k = 0; k < ref_users.length; k++) {
                                ...
                                if(player)
                                {
                                    let playingCount = player[0].online_multiplayer.played + player[0].friend_multiplayer.played;
                                    if(playingCount >= 5)
                                    {
                                        add_points += 200;
